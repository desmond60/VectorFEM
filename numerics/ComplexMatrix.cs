...
public class ComplexMatrix : ICloneable
{
    ...
    //: Перегрузка сложение матриц 
    public static ComplexMatrix operator +(Matrix<double> mat1, ComplexMatrix mat2) {
        var result = new ComplexMatrix(mat1.Dim);
        for (int i = 0; i < result.Dim; i++)
            for (int j = 0; j < result.Dim; j++)
                result[i, j] = mat1[i, j] + mat2[i, j];
    ...
    public static ComplexMatrix operator +(ComplexMatrix mat1, Matrix<double> mat2) => mat2 + mat1;
